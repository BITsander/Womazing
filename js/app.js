 ...
 /*! smooth-scroll v16.1.3 | (c) 2020 Chris Ferdinandi | MIT License | http://github.com/cferdinandi/smooth-scroll */            window.Element && !Element.prototype.closest && (Element.prototype.closest = function(e) {
    ...
    (() => {
        ...
        function initSliders() {
            if (document.querySelector(".thumb__slider")) {
                const thumbSlider = new core(".thumb__slider", {
            ...
            if (document.querySelector(".swiper-team__slider")) new core(".swiper-team__slider", {
                modules: [ Navigation, Pagination ],
                observer: true,
                observeParents: true,
                slidesPerView: 1,
        ...
        da.init();
        const wrapperMenu = document.querySelector(".header__menu-wrapper");
        var scrolled;
        window.onscroll = function() {
            scrolled = window.pageYOffset || document.documentElement.scrollTop;
            if (scrolled > 20) wrapperMenu.classList.add("_scrolled"); else wrapperMenu.classList.remove("_scrolled");
        };
        function vibor(parametr, activites) {
            let par = document.getElementsByClassName(parametr);
            let actives = document.getElementsByClassName(activites);
            for (let i = 0; par.length > i; i++) par[i].onclick = function() {
                var currentActive = actives[0];
                if (currentActive) currentActive.classList.remove(activites);
                if (currentActive !== this) this.classList.add(activites);
            };
        }
        vibor("list-color__item", "_active-color");
        vibor("list-size__item", "_active-size");
        window["FLS"] = true;
        isWebp();
        addLoadedClass();
        menuInit();
